...
%                                 end
    ...
    if any(poly_ok==2)
        ...
        for im=1:2  % im=1: run only the first three criteria to accept as many polygons based on this as possible.
            ...
            if im==1
                diagnostics=zeros(4*4*5,length(ixpo{im}));
            elseif im==2
                diagnostics2=zeros(4*4*5,length(ixpo{im}));
            ...
            for ik=1:length(ixpo{im})
                ...
                polylog=cell(4*4*5,1);
                ctr=0;
                for iv1=1:4     % four extreme boundary vertices
                    for iv2=1:4 % four extreme boundary vertices
